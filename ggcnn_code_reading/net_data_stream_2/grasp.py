...
class Grasp:
    '''
    定义一个抓取框处理类，主要功能是将由四个角点坐标定义的原始的抓取框
    提取转化为训练所定义的表征信息，如中心位置，面积角度等，
    并根据图像处理需求完成一些相应的其他功能
    '''
    ...
    @property#类装饰器，可以让一个类的方法以属性的方式被调用，后面无需加()
    def center(self):
        '''
        :功能          : 计算本类中所包含的抓取框的中心点
...
class Grasps:
    '''
    定义一个多抓取框处理类，主要功能是从原始的标注文件中读出多个抓取框
    并将其构建成多个单一的抓取框Grasp类，同时能够对这些属于同一对象的多个抓取框对象
    进行一些数据的统一集成处理'''
    ...
    @classmethod
    def load_from_cornell_filles(cls, cornell_grasp_files):
        '''
        ...
        with open(cornell_grasp_files,'r') as f:
            while True:
                ...
                grasp_rectangle = np.array([str2num(point0),
                               ...
                               str2num(point2),
                               str2num(point3)])
                grasp_rectangles.append(Grasp(grasp_rectangle)) # 对每一个grasp_rectangle构造一个Grasp类用于以后调用抓取位姿参数
            
            return cls(grasp_rectangles) # 将多个抓取框Grasp类集成到一个Grasps类，且Grasp类是Grasp类的列表
    ...
    def generate_img(self, pos = True, angle = True, width = True, shape = (480, 640)):
        ...
        :参数 shape :tuple用于确定图像的边界，确定在多大的范围内生成映射图
